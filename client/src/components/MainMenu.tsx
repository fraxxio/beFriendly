...
export default function MainMenu({ setIsPaired, socket, usernames, setUsernames }: MainMenuProps) {
  ...
  return (
    <section className='h-[70vh] container'>
      ...
      <div className='w-[50%] mx-auto pt-12 max-sm:w-full'>
        ...
        <form
          onSubmit={hSubmit}
          className='flex flex-col gap-2 pt-12 w-[70%] mx-auto text-center max-[810px]:w-full'
        >
          ...
          >
            {loading ? (
              <div className='flex items-center gap-2 justify-center max-sm:text-sm max-[300px]:text-xs '>
                Looking for a friend... <Search className='animate-bounce shrink-0' size={18} />
              </div>
            ) : (
              <div className='flex items-center gap-2 justify-center'>
                Find Friend <Search size={18} className='shrink-0' />
              </div>
            )}
          </button>
